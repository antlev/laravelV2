...
class SessionGuard implements StatefulGuard, SupportsBasicAuth
{
    ...
    public function getRoles(){ //recupere le role de la persone actuel (session)
        $roles = DB::select('select id_role from  user_roles where user_id = "'.$this->id().'"');
        return $roles;
    }
    public function isAdmin(){ //verifie si la personne est admin 
        $user = $this->user();
        $admin = (!isset($user->admin)) ? 0 : $user->admin;
        return $admin;
    }

    public function getNbMsg($id) {
$nb = 0;
        $user = DB::table('users_message')
...
return $nb;
    ...
    public function getMsg($id) {
        ...
        foreach($message as $messages) {
            $id = $messages->id;
            $to = $messages->destinataire;
            $from = $messages->emetteur;
            $message = $messages->message;
            $date = $messages->date;
            array_push($message_out, array('id'=>$id,'destinataire'=>$to,'emetteur'=>$from,'message'=>$message,'date'=>$date));
        }
        return $message_out;
    }
  ...
  public function getPhotoByid($id) { 
    $user = DB::table('users')->where('id', $id)->first();
      try {
          ...
          if ($dbphoto == "") {
              $dbphoto = "http://opstatic.com/img/profile/default_user.jpg";
          }
          return $dbphoto;
      }
      catch(\Exception $ex) {
...
public function getSexebyId($id) { //recupere le nom de la personne grace a son id
  ...
  public function getSportbyId($id) { //recupere le nom de la personne grace a son id
