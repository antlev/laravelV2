...
<img id="imguser" name="myimage" width=40% src="{{ Auth::getPhotoById(Auth::id()) }}">
<br><br>
...
<input type="text" class="form-control" id="pseudo" value="{{ ucfirst(Auth::user()->name) }}" disabled>
...
@if(Auth::user()->civilite != 'homme') <option value="homme">Homme</option> @endif
@if(Auth::user()->civilite != 'femme')<option value="femme">Femme</option> @endif
</select>
<span class="input-group-addon">
...
@foreach(Auth::getAllSports() as $sport)  

<option value="test">{{ $sport->nom_sports }}</option>
@endforeach
</select>
<span class="input-group-addon">
...
Ville : 
<div class="input-group">
<input type="text" class="form-control" id="ville"  value="{{ ucfirst(Auth::user()->ville)}}" disabled>
<span class="input-group-addon">
